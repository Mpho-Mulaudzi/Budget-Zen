...
public class NativeAdStreamHandler implements EventChannel.StreamHandler {
    ...
    public void onListen(Object arg, final EventChannel.EventSink event) {
        ...
        if (controller != null) {
            controller.setReportUrlListener(new ReportUrlListenerImpl(context, event));
    ...
    static class NativeAdLoadedListenerImpl implements NativeAd.NativeAdLoadedListener {
        ...
        public void onNativeAdLoaded(NativeAd nativeAd) {
            ...
            event.success(ToMap.fromArgs("reportUrlEvent", "reportFailed"));
            HMSLogger.getInstance(context).sendSingleEvent("reportUrlEvent");
    ...
    static class ReportUrlListenerImpl implements ReportUrlListener {
        private final Context context;

        private EventChannel.EventSink event;

        ReportUrlListenerImpl(Context context, EventChannel.EventSink event) {
            this.context = context;
            this.event = event;
        }
        ...
        public void reportSuccess() {
            HMSLogger.getInstance(context).startMethodExecutionTimer("reportSuccess");
            event.success(ToMap.fromArgs("reportUrlEvent", "reportSuccess"));
            HMSLogger.getInstance(context).sendSingleEvent("reportSuccess");
        }

        @Override
        public void reportFailed(String url, int errorCode) {
            HMSLogger.getInstance(context).startMethodExecutionTimer("reportFailed");
            Log.e(TAG, "reportFailed " + errorCode + " url: " + url);
            event.success(ToMap.fromArgs("reportUrlEvent", "reportFailed", "errorCode", errorCode, "url", url));
            HMSLogger.getInstance(context).sendSingleEvent("onNativeAdFailed", "errorCode:" + errorCode);
        }
    }
