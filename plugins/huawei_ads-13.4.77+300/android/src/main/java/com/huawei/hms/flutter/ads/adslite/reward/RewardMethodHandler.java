...
public class RewardMethodHandler implements MethodChannel.MethodCallHandler {
    ...
    public void onMethodCall(@NonNull final MethodCall call, @NonNull final Result result) {
        switch (call.method) {
            ...
            case "sendBiddingSuccess":
                sendBiddingSuccess(call, result);
                break;

            case "sendBiddingFailed":
                sendBiddingFailed(call, result);
                break;
            default:
    ...
    private void sendBiddingSuccess(MethodCall call, Result result) {
        Log.i("TAG", "received success");
        HMSLogger.getInstance(context).startMethodExecutionTimer("sendBiddingSuccess");
        Integer id = FromMap.toInteger("id", call.argument("id"));
        HmsRewardAd hmsRewardAd = HmsRewardAd.get(id);
        hmsRewardAd.sendBiddingSuccess(call.argument("sendBiddingSuccess"), new ReportUrlListener() {
            @Override
            public void reportSuccess() {
                Log.i("TAG", "RewardMethodHandler report success");
            }

            @Override
            public void reportFailed(String s, int i) {
                Log.i("TAG", "RewardMethodHandler report failed: " + s);
            }
        });
    }

    private void sendBiddingFailed(MethodCall call, Result result) {
        Log.i("TAG", "received failed");
        HMSLogger.getInstance(context).startMethodExecutionTimer("sendBiddingFailed");
        Integer id = FromMap.toInteger("id", call.argument("id"));
        HmsRewardAd hmsRewardAd = HmsRewardAd.get(id);
        hmsRewardAd.sendBiddingFailed(call.argument("sendBiddingFailed"), new ReportUrlListener() {
            @Override
            public void reportSuccess() {
                Log.i("TAG", "RewardMethodHandler report success");
            }
