...
public class SplashStreamHandler implements EventChannel.StreamHandler {
    ...
    public void onListen(Object arg, final EventChannel.EventSink event) {
        ...
        if (splash != null && splash.isLoading()) {
            ...
            ReportUrlListener reportUrlListener = new SplashReportUrlListenerImpl(context, event);
            splash.setDisplayListener(displayListener);
            splash.setReportUrlListener(reportUrlListener);
    ...
    static class SplashReportUrlListenerImpl implements ReportUrlListener {
        private final Context context;
        private EventChannel.EventSink event;

        SplashReportUrlListenerImpl(Context context, EventChannel.EventSink event) {
            this.context = context;
            this.event = event;
        }
        ...
        public void reportSuccess() {
            ...
            Log.i(TAG, "listener reportSuccess");
        ...
        public void reportFailed(String s, int i) {
            ...
            Log.i(TAG, "listener reportFailed");
            event.success(ToMap.fromArgs("reportUrlEvent", "reportFailed"));
            HMSLogger.getInstance(context).sendSingleEvent("reportFailed");
        }
    }
