...
class NativeAdController {
  ...
  ReportUrlListener? reportUrlListener;
  ...
  /// Reports the bidding success result.
  Future<void> sendBiddingSuccess(Map<String, dynamic> sendSuccessInfo) async {
    return await _channel.invokeMethod('sendBiddingSuccess', sendSuccessInfo);
  }

  /// Reports the bidding failure result.
  Future<void> sendBiddingFailed(Map<String, dynamic> sendFailInfo) async {
    return await _channel.invokeMethod('sendBiddingFailed', sendFailInfo);
  }
  ...
  void _startListening() {
    _listenerSub = _streamNative.receiveBroadcastStream(id).listen(
      (dynamic channelEvent) {
        print('channelEvent: ${channelEvent}');
        String? reportUrlEventValue = channelEvent['reportUrlEvent'];
        final Map<dynamic, dynamic> argumentsMap = channelEvent;
        final AdEvent? event = Ads.toAdEvent(argumentsMap['event']);
        ReportUrlEvent? reportUrlEvent;
        if (reportUrlEventValue != null) {
          print('reportUrlEventValue: $reportUrlEventValue');
          reportUrlEvent = Ads.toReportUrlEvent(reportUrlEventValue);
        }

        if (reportUrlEvent != null && reportUrlListener != null) {
          reportUrlEvent == ReportUrlEvent.failed
              ? reportUrlListener!(reportUrlEvent)
              : reportUrlListener!(reportUrlEvent);
        }
        switch (reportUrlEvent) {
          case ReportUrlEvent.success:
            print('success from switch');
            break;

          case ReportUrlEvent.failed:
            print('failed from switch');
            break;
